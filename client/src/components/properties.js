...
export default function Properties(){
  ...
  return (
    <div>
      ...
      deathday
      <GetInput value = {data.deathday}  name = {"deathday"} disabled={true}/>    
      <div>
      <h4>remark</h4>
      <input type="text" className="textarea"  value = {"" ||inputData[data.id]}  onChange={(e)=>handleChange(e)} name = {data.id} />
      
      <div className="button">
      <button className="button-3" onClick={()=>submitToProperties()}>save</button>
      <ToastContainer />
      </div>
        </div>
      <a className="button-33" href='/home' rel="noreferrer">Back</a>
